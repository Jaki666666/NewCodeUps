jQuery(function ($) { // この中であればWordpressでも「$」が使用可能になる
    ...
    const loadingSecond = document.querySelector('#loading-second');
    const mvTitle = document.querySelector('.mv__title');
    const mvImgs = document.querySelectorAll('.mv__picture');
    ...
    $(function () {
        if ($('.top-mv').length) {
            ...
            tl
                .to(loadingSecond, { translateY: '-5%', duration: 1.2, delay: .3 })
                ...
                .to(loadingSecond, { autoAlpha: 0, duration: 2.7 }, '+=3')
                .to(loadingSecond, { display: 'none' }, '-=2.7')
                .add(() => {
    ...
    // ＝＝＝＝＝＝headerリンクアニメーション＝＝＝＝＝＝
    document.querySelectorAll('.js-splitText').forEach((splitTarget) => {
        const headerLink = splitTarget.closest('.header__item');

        let newText = ''; // 生成する要素を格納するための変数
        const spanText = splitTarget.textContent; // ターゲットの中身を取得

        // span要素で文字を囲む
        spanText.split('').forEach((char, index) => {
            newText += `<span class="char-${index + 1}">${char}</span>`;
        });

        splitTarget.innerHTML = newText; // ターゲットに生成した要素を挿入

        let spans = splitTarget.querySelectorAll('span');
        let activeAnimation;
        ...
        headerLink.addEventListener('mouseover', function () {
            ...
            spans.forEach((span, index) => {
                activeAnimation.to(span, { color: '#34c9c4', delay: index * 0.02 }, 0);
            });
        });
        ...
        headerLink.addEventListener('mouseout', function () {
            ...
            spans.forEach((span, index) => {
                activeAnimation.to(span, { color: '#04047a', delay: (spans.length - 1 - index) * 0.02 }, 0);
            });
        });
    });
    ...
    function drawerEvent() {
        ...
        if (drawerMenu.classList.contains('drawer--opened')) {
            gsap.to(drawerMenu, { visibility: 'visible' })
            gsap.to(header, { background: '#04047a' })
            gsap.to(drawerMenu, { opacity: 1 })
            // ドロワーメニューが閉じる時
        } else {
            const tl = gsap.timeline();
            tl
                .to(drawerMenu, { opacity: 0 })
                .to(header, { background: '#fff' }, '<')
                .to(drawerMenu, { visibility: 'hidden', duration: .1 })
        }
    }
    ...
    for (let i = 0; i < drawerLinks.length; i++) {
        drawerLinks[i].addEventListener('click', () => {
            headerClickNumber = 0
        });
    }
    ...
    // ＝＝＝＝＝＝セクションタイトルのアニメーション＝＝＝＝＝＝
    const initials = document.querySelectorAll('.js-initial');

    initials.forEach((initial) => {
        const sectionTitleEnglish = initial.parentNode;
        const sectionTitle = sectionTitleEnglish.parentNode;
        const restLetter = initial.nextElementSibling;
        const sectionTitleJapanese = sectionTitle.children[1];;
    ...
    // ＝＝＝＝＝＝information swiper＝＝＝＝＝＝
    const informationSwiper = new Swiper('.js-information-swiper', {
        loop: true,
    ...
    // ＝＝＝＝＝＝voice画像アニメーション＝＝＝＝＝＝
    const AnimationImgs = document.querySelectorAll('.js-img-animation');

    AnimationImgs.forEach((AnimationImg) => {
        const AnimationBg = AnimationImg.parentElement.querySelector('.js-bg-animation');
        const tl = gsap.timeline({
            scrollTrigger: {
                trigger: AnimationImg, // トリガーとなる要素を指定
        ...
        tl
            .to(AnimationBg, { x: '0%' })
            .to(AnimationImg, { autoAlpha: 1 })
            .to(AnimationBg, { x: '-110%' });
    })

    // ＝＝＝＝＝＝price画像アニメーション＝＝＝＝＝＝
    $(function () {
        if ($('#price-animation-img').length) {
            const PriceAnimationImg = document.querySelector('#price-animation-img');
            const PriceAnimationBg = document.querySelector('#price-bg-animation');

            const tl = gsap.timeline({
                scrollTrigger: {
                    trigger: PriceAnimationImg, // トリガーとなる要素を指定
                    start: "top bottom", // トリガーが画面のどの位置で開始するか指定
                }
            });

            tl
                .to(PriceAnimationBg, { x: '0%' })
                .to(PriceAnimationImg, { autoAlpha: 1 })
                .to(PriceAnimationBg, { x: '-110%' });
        }
    });

    // ＝＝＝＝＝＝ページトップボタンの処理＝＝＝＝＝＝
    const topBtn = document.querySelector('#top-btn');
    const topBtnFront = document.querySelector('#top-btn__wrapper--front');
    const topBtnBack = document.querySelector('#top-btn__wrapper--back');
    ...
    $(function () {
        $('a[href*="#"]').on('click', function () {
            ...
            const scrollPosition = scrollToTarget.offset().top - headerHeight - 20;
            $('html, body').animate({
    ...
    $(function () {
        if ($('.accordion').length) {
            ...
            accordionTitles.forEach(accordionTitle => {
                let accordionItems = accordionTitle.nextElementSibling
                accordionTitle.addEventListener('click', () => {
                    ...
                    if (accordionItems.classList.contains('open')) {
                        gsap.to(accordionItems, { height: 'auto', duration: .3 })
                    } else {
                        gsap.to(accordionItems, { height: 0, duration: .3 })
                    }
                })
            });
        }
    ...
    $(function () {
        if ($('.question-list').length) {
            const title = document.querySelectorAll('.question-list__title');
            const accordion = document.querySelector('.question-list__accordion');

            title.forEach(titleEach => {
                let explanation = titleEach.nextElementSibling
